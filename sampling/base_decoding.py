...
@torch.no_grad()
def autoregressive_generate_mm(
    ...
    """
    cache = None
    inputs = inputs_list[0][0].tolist()
    pixel_values = inputs_list[1].to(model.device, dtype=torch.float32)
    ...
    for curr in range(prompt_len, total_len):
        o = model(input_ids=input_ids[..., :curr], pixel_values=pixel_values, past_key_values=cache, use_cache=use_cache)
        logits = o.logits[..., -1, :]  # [1, vocab_size]
        probs = logits_processor(logits)  # [1, vocab_size]
        x = logits_processor.sample(probs)  # [1, 1]
        input_ids[0, curr] = x
